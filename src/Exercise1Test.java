...
class Exercise1Test {
    ...
    void _version() {
        System.out.println("Test version 2.1 (2023-03-25)");
    }
    ...
    void bookCorrectPriceForBoundBook(double price, double expPrice, double expPricePlusVat) {
        ...
        assertEquals(expPrice, book.getPrice(), 0.005, BOUND_BOOK_INCORRECT_VALUE_EXCL_VAT);
        assertEquals(expPricePlusVat, book.getPriceWithVAT(), 0.005, BOUND_BOOK_INCORRECT_VALUE_INCL_VAT);
    }
    ...
    void bookCorrectPriceForUnboundBook(double price, double expPrice, double expPricePlusVat) {
        ...
        assertEquals(expPricePlusVat, book.getPriceWithVAT(), 0.005, UNBOUND_BOOK_INCORRECT_VALUE_INCL_VAT);
    }
    ...
    void bookCorrectToString(String title, String author, double price, boolean bound) {
        ...
        var parts = Set.of(title, author, String.valueOf(price * (bound ? 1.30 : 1.0)), String.valueOf(bound));
    ...
    void bookImplementsCorrectInterface() {
        ...
        var implementsInterface = PriceableWithVAT6.class.isAssignableFrom(clazz);

        assertEquals(1, interfaces.length, "Enligt klassdiagrammet ska klassen implementera ett gränssnitt.");
    ...
    void itemHasCorrectMember() {
        ...
        assertTrue(isFinal, "Den enda instansvariabeln ska inte kunna ändra värde.");
    }
    ...
    void itemHasCorrectMethods() {
        ...
        assertEquals(1, pubMethods, "Enligt klassdiagrammet ska klassen bara ha en publika metod utöver konstruktorn.");

        Method method = null;
        try {
            method = clazz.getDeclaredMethod("getName");
        } catch (NoSuchMethodException e) {
            fail("Kunde inte hitta metoden getName.");
        }
    ...
    void itemImplementsVat() {
        var clazz = Item.class;
        var interfaces = clazz.getInterfaces();
        var implementsInterface = Priceable.class.isAssignableFrom(clazz);
    ...
    @CsvSource({"2021,10,200,210,262.5", "2021,5,200,110,137.5", "2021,1,200,30,37.5", "2021,0,200,10,12.5"})
    ...
    @CsvSource({"2020,10,200,215,268.75", "1950,10,200,565,706.25"})
    ...
    void recordingGetsCorrectMinimumValue() {
        ...
        assertEquals(24, item.getPrice(), "Fel värde för Longplay med condition 1.");
    ...
    void recordingImplementsCorrectInterface() {
        var clazz = Recording.class;
        var interfaces = clazz.getInterfaces();
        var implementsInterface = PriceableWithVAT25.class.isAssignableFrom(clazz);
    ...
    void recordingImplementsCorrectMethods() {
        ...
        var methodNames = Arrays.stream(methods).map(m -> m.getName()).toList();
        assertTrue(methodNames.contains("getPrice"), "Metoden getPrice saknas");
        assertTrue(methodNames.contains("getType"), "Metoden getType saknas");
        assertTrue(methodNames.contains("toString"), "Metoden toString saknas");
        assertTrue(methodNames.contains("getYear"), "Metoden getYear saknas");
        assertTrue(methodNames.contains("getArtist"), "Metoden getArtist saknas");
        assertTrue(methodNames.contains("getCondition"), "Metoden getCondition saknas");
    }
    ...
    void testOrderMethods() {
		...
		*/
        ...
        assertEquals(657.2, order.getTotalValuePlusVAT(), 0.005, ORDER_INCORRECT_VALUE_INCL_VAT);
		...
		*/
        ...
        assertEquals(870.0, order.getTotalValue(), ORDER_INCORRECT_VALUE_EXCL_VAT);
        assertEquals(1011.5, order.getTotalValuePlusVAT(), ORDER_INCORRECT_VALUE_INCL_VAT);

        part = Set.of("Book", "Beethoven: a biography", "Holmqvist", "false", "400.0", "424.0");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));

        part = Set.of("LP", "Giant Steps", "John Coltrane", "1959", "10", "100.0", "420.0", "525.0");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));

        part = Set.of("CD", "Kind of Blue", "Miles Davis", "1959", "5", "100.0", "50.0", "62.5");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));
        ...
        assertEquals(495.0, order.getTotalValue(), ORDER_INCORRECT_VALUE_EXCL_VAT);
        assertEquals(618.75, order.getTotalValuePlusVAT(), 0.005, ORDER_INCORRECT_VALUE_INCL_VAT);

        part = Set.of("CD", "Punisher", "Phoebe Bridgers", "2020", "10", "200.0", "250.0");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));

        part = Set.of("LP", "What Kinda Music", "Tom Misch", "2020", "10", "150.0", "165.0", "206.25");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));

        part = Set.of("LP", "Little Oblivions", "Julien Baker", "2021", "10", "120.0", "150.0");
        assertTrue(part.stream().allMatch(receipt::contains), String.format(message, part, receipt));
    }

    @Test
    @org.junit.jupiter.api.Order(1)
    @DisplayName("Kontrollerar att gränssnitten Priceable, PriceableWithVAT6 och PriceableWithVAT25 är korrekta.")
    void vatInterfaceTests() {
        assertTrue(ReflectionUtils.isAbstract(Priceable.class));
        assertTrue(ReflectionUtils.isAbstract(PriceableWithVAT6.class));
        assertTrue(ReflectionUtils.isAbstract(PriceableWithVAT25.class));

        assertEquals(3, Priceable.class.getDeclaredMethods().length, "Gränssnittet Priceable ska ha exakt tre metoder.");

        class TEST_VAT6 implements PriceableWithVAT6 {
            public double getPrice() { return 0;}
        }
        assertEquals(0.06, new TEST_VAT6().getVAT(), "Metoden getVAT ger inte rätt värde för gränssnittet PriceableWithVAT6.");

        class TEST_VAT25 implements PriceableWithVAT25 {
            public double getPrice() { return 0;}
        }
        assertEquals(0.25, new TEST_VAT25().getVAT(), "Metoden getVAT ger inte rätt värde för gränssnittet PriceableWithVAT25.");
    }
}